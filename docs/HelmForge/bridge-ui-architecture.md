...
Phase 17A update: Product polish keeps the existing Bridge/UI authority split intact. The UI sweep distinguishes status chips from action buttons, keeps page content in scroll areas, improves Live Overlay dialog sizing, labels Flight Recorder preview as metadata-only with no capture or encoding, and replaces loose vJoy/live-output wording with output-intent and verified-write terminology. It does not add hardware polling, vJoy writes, output verification changes, Bridge lifecycle management, recorder capture/encoding, installer behavior, auto-save, or runtime activation.

Phase 17B update: Interaction polish keeps the same authority split and focuses on update discipline. `HelmForgeShell` owns a shared `DiagnosticsCollector` for page-switch timing and heavy-page update counters. Live Monitor and Effective Response Stack skip hidden timer ticks and record those skips, while graph widgets update existing pyqtgraph items instead of clearing and rebuilding plots. These metrics are observational UI/app performance truth only; they do not poll hardware, write vJoy, verify output, or activate runtime.

Phase 17C update: Final product QA keeps the same authority split and freezes Phase 17. The pass confirms page/dialog smoke behavior, runtime truth copy, readiness proof fields, and Phase 18 handoff requirements. Phase 17 is now complete. The next prompt-book phase is Phase 18: Packaging, Installer, Icons, and User Data Locations. Phase 17C does not implement packaging, installer scripts, icon conversion, user data migration, Bridge lifecycle management, or runtime activation.

Phase 18B update: Packaging now has a PyInstaller one-folder build path at `packaging/dist/HelmForge/HelmForge.exe` plus the smoke command `packaging/dist/HelmForge/HelmForge.exe --smoke-exit-ms 250`. This packages the UI entry point without changing the Bridge/UI authority split. The packaged app must preserve simulation fallback, telemetry truth, and the Phase 16 Full Live Runtime Ready gate. Phase 18B does not add an installer, Start Menu shortcut, Desktop shortcut, uninstall entry, driver installer launch, automatic Bridge launch, Bridge lifecycle controls, new hardware polling, vJoy/output behavior changes, or runtime activation.

Phase 18D update: Final packaging QA verifies the one-folder build path, installer script metadata, icon-missing truth, LocalAppData user-data separation, and packaged smoke behavior while keeping the UI/Bridge authority split intact. Phase 18 is now complete. Phase 19 should validate the full product against the prompt book.
