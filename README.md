...
Phase 16D completes Runtime End-to-End Live Mode by adding the central Full Live Runtime Ready evaluator. Full Live Runtime Ready requires fresh physical input, successful pipeline processing, guarded real output verification, explicit output loop enable/running according to policy, fresh Bridge telemetry/runtime_frame, and no safety stop or blocking error. It is never inferred from vJoy detection alone, physical HOTAS detection alone, output intent generation alone, fake/mock verification, fake output loop, stale telemetry, or UI process presence. Phase 16 is now complete. The next prompt-book phase is Phase 17: Product Polish, Layout QA, and Motion, and Phase 17 must preserve this runtime truth/readiness gate.

Phase 17A begins Product Polish, Layout QA, and Motion with a visual consistency sweep only. It improves scroll-surface consistency, status chip/action button distinction, disabled action styling, Live Overlay dialog sizing, Flight Recorder metadata-only copy, Help / Docs coverage for Live Overlay and Flight Recorder, and output-intent terminology. It does not add hardware polling, vJoy writes, output verification changes, Bridge lifecycle management, recorder capture/encoding, packaging/installer behavior, auto-save, or runtime activation.

Phase 17B continues Product Polish, Layout QA, and Motion with interaction smoothness and performance discipline only. It keeps heavy pages reused, records shared page-switch/update/graph timings, counts hidden Live Monitor and Effective Response Stack update skips, updates plot data in place to reduce flicker, and keeps Live Overlay refresh timers stopped when hidden. It does not add hardware polling, vJoy writes, output verification changes, Bridge lifecycle management, recorder capture/encoding, packaging/installer behavior, auto-save, or runtime activation.

Phase 17C completes Product Polish, Layout QA, and Motion with a final product QA sweep and Phase 18 readiness census. Phase 17 is now complete. The next prompt-book phase is Phase 18: Packaging, Installer, Icons, and User Data Locations. Phase 17C does not implement packaging, installer scripts, icon conversion, user data migration, hardware polling, vJoy writes, output verification changes, Bridge lifecycle management, recorder capture/encoding, packaging/installer behavior, auto-save, or runtime activation. Phase 18 must preserve runtime truth behavior and simulation mode first launch.

Phase 18A begins Packaging, Installer, Icons, and User Data Locations with foundation only. It adds `packaging/` documentation, a dry-run capable `packaging/build_release.ps1` script, PyInstaller one-folder build notes, Inno Setup handoff notes, LocalAppData user data path helpers, and resource path helpers. Phase 18A does not implement a full installer, final release build, driver installer launch, automatic Bridge launch, Bridge lifecycle management, new hardware polling, vJoy/output behavior changes, recorder capture/encoding, game injection, graphics API hooking, cloud AI/LLM behavior, auto-save, or unsupported runtime activation.

Phase 18B adds the first real PyInstaller one-folder build path. The expected packaged executable is `packaging/dist/HelmForge/HelmForge.exe`, and the packaged smoke command is `packaging/dist/HelmForge/HelmForge.exe --smoke-exit-ms 250`. Phase 18B does not create an installer, does not launch driver installers, does not add shortcuts or uninstall entries, and does not add Bridge lifecycle management. The packaged app must preserve simulation mode and the Phase 16 Full Live Runtime Ready gate when HOTAS/vJoy is missing.

Phase 18D completes Packaging, Installer, Icons, and User Data Locations with final packaging QA. Phase 18 is now complete. The next prompt-book phase is Phase 19: Final Integration Kraken / Full Acceptance Sweep. Phase 18D does not add runtime authority, Bridge lifecycle management, new hardware polling, vJoy/output behavior changes, recorder capture/encoding, game injection, graphics API hooking, cloud AI/LLM behavior, auto-save, or unsupported runtime activation.
...
Source launch remains supported:
...
Phase 18A packaging foundation lives under `packaging/`. Phase 18B makes the selected first build path a PyInstaller one-folder build. Phase 18C adds icon-aware build metadata and an Inno Setup installer script at `packaging/inno/helmforge.iss`; it defines a Start Menu shortcut, optional Desktop shortcut, optional launch-after-install task, and uninstall metadata. Phase 18D finalizes packaging QA and Phase 19 readiness. The installer does not install drivers, does not install vJoy, and does not manage Bridge lifecycle. Run the conservative dry-run script from the repo root:

```powershell
powershell -NoProfile -ExecutionPolicy Bypass -File .\packaging\build_release.ps1 -DryRun
```

Phase 18D does not add runtime authority and does not claim a final signed release. Packaged builds must preserve simulation mode, missing HOTAS/vJoy guidance, telemetry truth, and the Phase 16 Full Live Runtime Ready gate. Phase 18 is now complete; Phase 19: Final Integration Kraken / Full Acceptance Sweep should validate the whole product against the prompt book.

Phase 19C correction status: the reviewed Kraken artifact is `.artifacts/phase19b_kraken/20260508T012841Z/` with 19 pass, 0 fail, and 1 skipped installer compile because `ISCC.exe` is unavailable. `assets/app_icon.ico is missing`, so icon embedding remains a known packaging blocker. No installer binary is claimed. Real hardware Full Live Runtime Ready proof is not claimed, and packaging smoke is not runtime readiness. Phase 19D is the final acceptance / release-candidate freeze.

Phase 19D final acceptance status: `docs/HelmForge/final-acceptance-report.md` classifies the current state as RC Ready With Known Non-Blocking Gaps if the missing icon, skipped Inno compile, installer install/uninstall QA, signed metadata, and live HOTAS/vJoy proof are outside this simulation-first RC's criteria. The latest Phase 19D Kraken rerun artifact is `.artifacts/phase19b_kraken/20260508T061208Z/` with 19 pass, 0 fail, and 1 skipped installer compile. The packaged build path remains `packaging/dist/HelmForge/HelmForge.exe`. `assets/app_icon.ico is missing`, packaged smoke is not runtime readiness, Full Live Runtime Ready remains governed by the Phase 16 proof gate, and there is no Bridge lifecycle management.

Expected Phase 18B packaged smoke command after a successful build:

```powershell
.\packaging\dist\HelmForge\HelmForge.exe --smoke-exit-ms 250
```

Phase 18C can compile the installer only when Inno Setup is available:

```powershell
powershell -NoProfile -ExecutionPolicy Bypass -File .\packaging\build_release.ps1 -BuildInstaller
```

User data is preserved by default on uninstall under `%LocalAppData%\HelmForge`; in installer docs, user data is preserved while app binaries install separately under `%LocalAppData%\Programs\HelmForge`.
