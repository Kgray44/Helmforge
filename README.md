...
The current rebuild state includes the Phase 7 Conditional Rules page/evaluator, Phase 6B Mapping editor polish, Phase 6 core tuning pages, Phase 5 Mapping page, Phase 4 PySide6 visual shell, Phase 2B Bridge/UI architecture boundary contracts, Phase 2A local runtime setup tooling, and the Phase 3 tuning math and signal pipeline. Phase 7 replaces the Conditional Rules placeholder with a screenshot-guided rule list/detail/editor and shared-core rule evaluation for simulated/future Bridge use. The current app still does not implement real HOTAS input polling, vJoy output writes, Effective Response Stack internals, Live Monitor internals, overlays, or installer packaging.
...
Phase 7 does **not** provide real support for that hardware. It does **not** implement real HOTAS polling or real vJoy output writes. No live runtime support should be claimed until a later phase implements and verifies it.
